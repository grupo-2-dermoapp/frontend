...
import {
  RegistroUsuarioBackendInterface,
  RegistroUsuarioFormInterface,
} from 'src/app/interfaces/registro-usuario.interface';
...
export class RegistroPage {
    ...
    this.registro = this.fb.group(
      {
        ...
        edad: [
          null,
          [Validators.required, Validators.max(99), Validators.min(18)],
        ],
        residencia: [
          '',
          [
  ...
  async registrarse() {
    if (this.registro.invalid) {
      Object.keys(this.registro.controls).forEach((control) => {
      ...
      this.registroService
        .registro(this.transformarParaBackend(this.registro.value))
        .subscribe({
          next: async (res) => {
  ...
  transformarParaBackend(
    registroUsuario: RegistroUsuarioFormInterface
  ): RegistroUsuarioBackendInterface {
    return {
